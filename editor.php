...
<body>
    ...
    <br>
    <br>
    <h1 style="text-align: center;"><strong>Редактирование новости</strong></h1>
    <br>
    ...
    <div class="chs">
        <div class="one">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 form-group mb-3" data-for="url">
                ...
                echo '<input type="urls" name="url" style="width: 80%; margin-left: 10%" placeholder="Ссылка на фото" data-form-field="url" class="form-control" value="'. $row['url'].'" id="url-form7-2">'                               
            ?>
            </div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 form-group mb-3" data-for="date">
                ...
                mysql_query ("set collation_connection='utf8_general_ci'");
                 ...
                 $result = mysqli_query($con, "SELECT * FROM `news` ORDER BY `number` DESC");
                $row = mysqli_fetch_assoc($result); // получаем все строки в цикле по одной
                echo '<input type="text" maxlength="20" name="date" style="width: 80%; margin-left: 10%" placeholder="Дата" data-form-field="date" class="form-control" value="'. $row['date'].'" id="date-form7-2">'                               
            ?>
            </div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 form-group mb-3" data-for="title">
                ...
                mysql_query ("set collation_connection='utf8_general_ci'");
                 ...
                 $result = mysqli_query($con, "SELECT * FROM `news` ORDER BY `number` DESC");
                $row = mysqli_fetch_assoc($result); // получаем все строки в цикле по одной
                echo '<input type="title" maxlength="30" name="title" style="width: 80%; margin-left: 10%" placeholder="Заголовок" data-form-field="title" class="form-control" value="'. $row['title'].'" id="title-form7-2">'                               
            ?>
            </div>
            <br>
            ...
            <?php 
                ...
                mysql_query ("set collation_connection='utf8_general_ci'");
                 ...
                 $result = mysqli_query($con, "SELECT * FROM `news` ORDER BY `number` DESC");
                $row = mysqli_fetch_assoc($result); // получаем все строки в цикле по одной
                echo '<textarea id="pole" style="width: 80%; margin-left: 10%; height: 30%" class="form-control" name="opis" cols="1" id="opis-form7-2" rows="5">'.$row['description'].'</textarea>'                               
            ?>
        </div>
    </div>
    <div class="dd">
    <div style="margin-left: 5px;" class="col-auto vhod knopkavhod mbr-section-btn align-center"><button><input type="button" onclick="otmena()" class="btn btn-secondary display-4" value="Отменить"></button></div>
            ...
            <div style="margin-right: 5px;" class="col-auto vhod knopkavhod mbr-section-btn align-center"><button><input type="submit" class="btn btn-secondary display-4" value="Применить изменения" name="request" onclick="prim()"></button></div>
    </div>
    </form>
    <br>
    <script>
